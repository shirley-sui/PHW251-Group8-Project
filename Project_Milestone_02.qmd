...
From the flu case rates and case severity datasets: - county - age_category - race_eth - dt_dx - dx_new - severe_new - susceptible - pop
...
selected_columns_sim_flu_LACounty_raw <- sim_flu_LACounty_raw[, c("dx_new", "severe_new", "susceptible")]

selected_columns_vax_rates_quarter_raw <- ca_vax_rates_quarter_raw[, c( "total_partial_vaccinated", "cumulative_fully_vaccinated", "cumulative_at_least_one_dose", "cumulative_unvaccinated", "cumulative_up_to_date_vax", "estimated_population")]

selected_columns_sim_flu_CA_new <- sim_flu_CA_new[, c("pop", "dx_new", "severe_new", "susceptible")]




summary(selected_columns_sim_flu_CA_new)
summary(selected_columns_sim_flu_LACounty_raw)
summary(selected_columns_vax_rates_quarter_raw)


range_values_CA_new <- sapply(selected_columns_sim_flu_CA_new, function(x) max(x) - min(x))
range_values_LACounty_raw <- sapply(selected_columns_sim_flu_LACounty_raw, function(x) max(x) - min(x))
range_values_vax_raw <- sapply(selected_columns_vax_rates_quarter_raw, function(x) max(x) - min(x))
...
char_sim_flu_LACounty_raw <- sim_flu_LACounty_raw[, c("race_eth", "age_category", "sex")]
                                                                   
char_sim_flu_CA_new <- sim_flu_CA_new[, c("race_eth", "age_category", "sex", "county")]
                                                                  
char_ca_vax_rates_quarter_raw <- ca_vax_rates_quarter_raw[, c("county_name", "demographic_category", "demographic_value")]

unique(char_sim_flu_LACounty_raw)
unique(char_sim_flu_CA_new)
unique(char_ca_vax_rates_quarter_raw)
