...
The data was obtained from the project github repository for the PHW251 R for Public Health course. The data we are using for this project is from a simulated data that represents flu outbreak and COVID-19 vaccine update in California from 2022 to 2023.
...
sim_flu_LACounty <- read_csv("scenario_1/sim_flu_LACounty_pop.csv")
...
sim_flu_CA_new <- sim_flu_CA_raw %>%
  mutate(dx_new = new_infections, recovered_new = new_recovered, susceptible = count_susceptible, infected_current = current_infected, infected_cumulative = cumulative_infected, recovered_cumulative = cumulative_recovered, severe_new = new_severe, severe_cumulative = cumulative_severe, age_category = age_cat, race_eth = race_ethnicity, dt_report = report_date, dt_dx = dt_diagnosis, population = pop) %>%
  select(county, dx_new, recovered_new, susceptible, infected_current, infected_cumulative, recovered_cumulative, severe_new, severe_cumulative, age_category, sex, race_eth, dt_report, dt_dx, population)
...
sim_flu_LACounty_raw <- select(sim_flu_LACounty_raw, c(dx_new, recovered_new, susceptible, infected_current, infected_cumulative, recovered_cumulative, severe_new, severe_cumulative, age_category, sex, race_eth, dt_report, dt_dx, population))
...
sim_flu_CA_col_types <- data.frame(
  coltypes_flu_CA = sapply(sim_flu_CA_new, typeof),
  stringsAsFactors = FALSE
)
...
# Select desired numeric data elements from each dataframe
num_sim_flu_LACounty_raw <- sim_flu_LACounty_raw[, c("dx_new", "severe_new", "susceptible", "population")]

num_vax_rates_quarter_raw <- ca_vax_rates_quarter_raw[, c( "total_partial_vaccinated", "cumulative_fully_vaccinated", "cumulative_at_least_one_dose", "cumulative_unvaccinated", "cumulative_up_to_date_vax", "estimated_population")]

num_sim_flu_CA_new <- sim_flu_CA_new[, c("dx_new", "severe_new", "susceptible", "population")]

# Print summary for each dataset to review mean and medians for the selected columns
summary(num_sim_flu_CA_new)
summary(num_sim_flu_LACounty_raw)
summary(num_vax_rates_quarter_raw)

# Print ranges for each of the selected columns from each dataset
range_values_CA_new <- sapply(num_sim_flu_CA_new, function(x) max(x) - min(x))
range_values_LACounty_raw <- sapply(num_sim_flu_LACounty_raw, function(x) max(x) - min(x))
range_values_vax_raw <- sapply(num_vax_rates_quarter_raw, function(x) max(x) - min(x))

print(range_values_CA_new)
print(range_values_LACounty_raw)
print(range_values_vax_raw)


# List the unique character values for each of the selected columns in each dataset
char_sim_flu_CA_new <- list(sim_flu_CA_new$race_eth, sim_flu_CA_new$age_category, sim_flu_CA_new$sex, sim_flu_CA_new$county)
char_sim_flu_LACounty_raw <- list(sim_flu_LACounty_raw$race_eth, sim_flu_LACounty_raw$age_category, sim_flu_LACounty_raw$sex)
char_ca_vax_rates_quarter_raw <- list(ca_vax_rates_quarter_raw$county_name, ca_vax_rates_quarter_raw$demographic_category, ca_vax_rates_quarter_raw$demographic_value)

# Print unique values from each of the selected columns
unique_CA_new <- lapply(char_sim_flu_CA_new, unique)
unique_LACounty_raw <- lapply(char_sim_flu_LACounty_raw, unique)
unique_vax_raw <- lapply(char_ca_vax_rates_quarter_raw, unique)

print(unique_CA_new)
print(unique_LACounty_raw)
print(unique_vax_raw)
```
...
-   Data elements that need cleaning or re-categorization
    ...
    -   Remove rows with NA values for some of the data
    -   In the COVID-19 vaccination dataset, convert demographic category + demographic value information to match the format from the simulated flu rates datasets. Some of the demographic categories also may need additional work to align them with the other two datasets (e.g. "Unknown/undifferentiated" gender)
    -   In the COVID-19 vaccination dataset, the age categories of "Under 5", "5-11", and "12-17" need to be combined to match the other two dataset categories
    -   dt_dx in the simulated flu rates LA County data needs to be converted to an R date column
