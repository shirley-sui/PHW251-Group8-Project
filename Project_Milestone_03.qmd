...
# Add column for yr_epiweek, calculated from dt_report
  mutate(yr_epiweek = paste(epiyear(dt_report), epiweek(dt_report), sep = "_")) %>%
...
# 2) add yr_epiweek column to dataframe
df_flu_LA_clean$yr_epiweek <- paste(epiyear(df_flu_LA_clean$dt_dx), epiweek(df_flu_LA_clean$dt_dx), sep = "_")
...
```{r flu - select demo/geo strata of interest - Nicole & Shirley}

# Group data by the county & age_category
df_flu_clean_aggregate <- df_flu_CA_clean %>% group_by(age_category,county)%>%
  summarise(sum_dx_new = sum(dx_new)) #sum dx_new

max(df_flu_clean_aggregate$sum_dx_new) # Find highest IR in data  
#highest IR is San Diego County: 18-49 age group for 606350 newly diagnosed individuals
 

head(df_flu_clean_aggregate)

# highest_incidence_rate <- df_flu_clean_aggregate %>%
#   filter(county == "San Diego County" & age_category == "18-49")

```

-   Aggregate the data into a new dataframe to include only one row per strata of interest

```{r flu - aggregate data, new df with one row per strata of interest -Nicole & Shirley}


# Define the strata of interest
strata_county <- "San Diego County"
strata_age_category <- "18-49"

# Use the aggregate function to calculate the sum of dx_new for the specified strata
aggregated_data <- aggregate(dx_new ~ county + age_category, data = df_flu_clean, FUN = sum)

# Filter the aggregated data to include only the strata of interest
strata_of_interest <- aggregated_data[aggregated_data$county == strata_county & aggregated_data$age_category == strata_age_category, ]
...
df_ca_vax_clean_clean <- df_ca_vax_clean %>%
...
df_ca_vax_clean_subset <- df_ca_vax_clean_subset %>%
  group_by(df_ca_vax_clean0$race_eth, df_ca_vax_clean0$age_category, df_ca_vax_clean0$sex) %>% 
  summarize(
