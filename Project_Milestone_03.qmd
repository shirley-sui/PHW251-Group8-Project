...
df_flu_clean_aggregate <- df_flu_CA_clean %>% group_by(age_category,county,yr_epiweek)%>%
...
# Add column for yr_epiweek, calculated from dt_admin
  mutate(yr_epiweek = paste(epiyear(dt_admin), epiweek(dt_admin), sep = "_"))
...
df_ca_vax_clean_subset <- df_ca_vax_clean_subset %>%
  group_by(age_category,county,yr_epiweek) %>% 
  summarize(
